...
class UIConsola:
    ...
    def ejecutar_turno_de_la_casa(self):
        ...
        if self.blackjack.casa_gano() == True:
            self.mostrar_manos(self.blackjack.cupier.mano, self.blackjack.jugador.mano)
            print("LA CASA HA GANADO EL JUEGO!")
            self.iniciar_nuevo_juego()
        else:
            while self.blackjack.casa_puede_pedir == True:
                self.blackjack.repartir_carta_a_la_casa()
        if self.blackjack.hay_empate():
            self.mostrar_manos(self.blackjack.cupier.mano, self.blackjack.jugador.mano)
            print("\n EL JUEGO A QUEDADO EN EMPATE")
            self.iniciar_nuevo_juego()
